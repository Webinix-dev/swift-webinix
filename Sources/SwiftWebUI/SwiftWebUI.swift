...
func swiftEventHandler(_ webinix_event_ptr: UnsafeMutablePointer<webinix_event_t>?) {
	let e = webinix_event_ptr?.pointee
...
public final class Window {
	...
	init(_ webinix_win: Int) {
		id = webinix_win
	}

	/// Binds a function to the Webinix window object or directly to a HTML elements click event.
...
public struct Event {
	...
	func response<T>(_ value: T) throws {
		...
		if value is String {
			...
			throw WebinixError.runtimeError("""
...
public func newWindowId(_ id: Int) -> Window {
	return Window(webinix_new_window_id(id))
}
...
/// Closes all open windows. `webinix_wait()` will return (break).
public func exit() {
	webinix_exit()
}
...
/// Should be used before `webinix_show()`.
/// - Parameter path: The full path to the local folder.
public func setRootFolder(_ path: String) throws {
	if !(path.withCString { p in
		webinix_set_default_root_folder(p)
	}) {
		throw WebinixError.runtimeError("error: failed to set default root folder")
	}
}

/// Set the SSL/TLS certificate and the private key content, both in PEM format.
/// This works only with `webinix-2-secure` library. If set empty Webinix will generate
/// a self-signed certificate.
/// - Parameters:
///   - certificate_pem: The SSL/TLS certificate content in PEM format.
///   - private_key_pem: The private key content in PEM format.
/// - Throws: `WebinixError.runtimeError` if the certifacte or the key are invalid.
public func setTLSCertificate(_ certifcatePem: String, _ privateKeyPem: String) throws {
	if !(certifcatePem.withCString { certPem in
		privateKeyPem.withCString { keyPem in
			webinix_set_tls_certificate(certPem, keyPem)
		}
	}) {
		throw WebinixError.runtimeError("error: failed to set TLS certificate")
	}
}
